...
- 历史实现曾在数据库不可用时回退到环境变量开发管理员账号；2026-09-23 本地源码已移除该回退，后台登录只接受启用的数据库管理员。修复版 API 已于 2026-09-24 发布到服务器，管理员签名密钥已轮换；保留的管理员需重新登录验收。
...
- 旧阶段的 10 个 API 自动化测试、3 个 MySQL 集成测试曾通过；2026-09-23 重新验证当前源码为 38 个单元测试和 9 个独立 MySQL 集成测试通过，详见 `docs/current-development-progress.md`。
