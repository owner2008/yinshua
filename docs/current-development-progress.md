...
- 安全收口（本地源码）：模拟微信登录默认关闭，只有非生产环境显式设置 `ALLOW_MOCK_WECHAT_LOGIN=true` 才能使用；旧模拟会员 token 在默认配置下被拒绝；手机号绑定改为必须携带会员 token 且仅更新当前用户。独立 H5 不再自动用固定测试身份登录，未登录时不显示会员资料表单。这些 API/H5 源码改动**尚未发布到服务器**。
...
- 后台默认账号：线上数据库中遗留的默认管理员账号已停用，另一个具有权限管理权限的管理员仍启用；本地源码已删除环境变量/默认密码回退和空角色全权限回退，seed 现在要求显式提供至少 16 位管理员密码，且不会重新启用已停用账号。源码尚未发布，不得直接重启会运行 seed 的线上 API 容器。见 `docs/admin-account-security-2026-09-23.md`。
- 2026-09-24 已在服务器隔离目录验收修复版 API：产品分类 200，模拟身份、无权限会员操作及默认管理员登录均为 401；候选进程已停止，未切换公网。验收中旧 API 曾因数据库短时不可达返回一次 500，随后恢复，原因待查。详见 `docs/8088-member-auth-safeguard-2026-09-23.md`。线上仍运行旧 API，8088 会员代理拦截不得移除。
- 阿里云安全运行镜像及仅替换 API 启动命令的 Compose 覆盖文件已在服务器旁路准备，镜像的 OpenSSL/Prisma 加载在隔离容器中通过；尚未重建线上容器。发布及回退条件见 `deploy/aliyun/README.md`。
