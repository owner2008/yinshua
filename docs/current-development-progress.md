...
- 独立 H5 已移除首页对后台私有 `/admin/product-templates` 的无效请求，并把旧浏览器标题改为公司名称。2026-09-24 安全版已上线 8088，随后修正首页和联系页的错误联系方式；手机、座机、双邮箱、官网和地址与正式 WordPress 官网统一。前台构建和类型检查通过，浏览器在桌面 1280px、手机 320px 验收无横向溢出，手机首页控制台 0 错误。线上仅替换 H5 入口及带哈希的 JS/CSS，保留后台、旧资源和回退备份，详见 `deploy/aliyun/README.md`。
- 安全收口：API 已于 2026-09-24 发布修复版，模拟微信登录默认关闭，旧模拟会员 token 默认被拒绝，手机号绑定必须携带会员 token 且只更新当前用户。独立 H5 也已发布取消自动模拟登录的版本，新构建忽略并删除旧本地测试会话；真实微信登录仍未实现/验收，代理拦截不得提前移除。
...
- 2026-09-24 阿里云 8088 已运行安全版 API：镜像 `localhost/qddflc-api-runtime:20260924`，命令 `node dist-auth-20260924/src/main.js`；模拟身份及无 token 会员操作直接访问 API 返回 401，公开目录和首页返回 200，代理仍对三个会员入口返回 403。WordPress 官网独立运行并返回 200。H5 已发布安全修复，但真实微信登录未端到端验收。
...
- 官网公网 DNS 已指向新服务器，但阿里云服务器的内网 DNS 于 2026-09-24 仍返回旧服务器；官网自身调用有本机映射，8088 容器名解析已单独修复。内网解析根因仍待阿里云排查，不要改旧 DNSPod 记录或付费清理缓存来代替排查。见 `docs/wordpress-site-and-domain-2026-09-23.md`。
...
- GitHub 授权已补齐 `workflow` scope；`master` 与 `codex/sync-from-aliyun-20260923` 已同步，新电脑克隆默认分支即可看到本轮服务器发布记录与源码。该状态不表示 WordPress 数据库正文、服务器生产凭据或仍未发布的 H5 构建已进入 Git。
