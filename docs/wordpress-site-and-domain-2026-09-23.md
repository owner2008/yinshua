...
- 2026-09-24 再次从服务器核对：公网 DNS `223.5.5.5` 返回 `39.106.169.147`，阿里云内网 DNS `100.100.2.136` 仍返回旧 CNAME `qddflc.ecs.xinshangxin.cn` 和 `47.104.13.73`。正式官网经新服务器直连 `/contact/` 返回 200 且含新增邮箱；8088 API 已单独固定 Podman 内网 DNS，不再依赖阿里云递归器解析容器名。阿里云内网解析来源仍待工单排查，尚未执行可能收费的缓存清除或全局 DNS 修改。
