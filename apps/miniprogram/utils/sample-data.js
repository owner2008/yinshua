...
function buildOptions(spec) {
  const rows = [];
  (spec.materials || []).forEach(([optionValue, optionLabel]) =>
    rows.push({ optionType: 'material', optionValue, optionLabel }),
  );
  (spec.processes || []).forEach(([optionValue, optionLabel]) =>
    rows.push({ optionType: 'process', optionValue, optionLabel }),
  );
  (spec.printModes || []).forEach(([optionValue, optionLabel]) =>
    rows.push({ optionType: 'print_mode', optionValue, optionLabel }),
  );
  (spec.shapes || []).forEach(([optionValue, optionLabel]) =>
    rows.push({ optionType: 'shape', optionValue, optionLabel }),
  );
  return rows;
}
...
const MATERIAL_PET = ['2', '透明膜'];
const MATERIAL_THERMAL = ['4', '热敏纸'];
const MATERIAL_PVC = ['5', '合成纸'];
const MATERIAL_FRAGILE = ['6', '易碎防伪纸'];
const MATERIAL_REMOVABLE = ['7', '可移除胶铜版纸'];

const PROCESS_LAMINATION = ['lamination', '覆膜'];
const PROCESS_DIE_CUT = ['die_cut', '模切'];
const PROCESS_UV = ['uv', '局部光油'];
...
const sampleProducts = [
  ...
  },
  {
    ...
    name: '日化美妆标签',
    code: 'COSMETIC-LABEL',
    description: '适合洗护、美妆、香氛等包装标签，支持覆膜、局部光油、烫金等工艺。',
    applicationScenario: '洗护瓶贴、精华标签、香氛包装',
    isHot: true,
    sort: 70,
    status: 'active',
